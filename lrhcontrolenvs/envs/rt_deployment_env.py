...
class RtDeploymentEnv(LRhcEnvBase):
    ...
    def _sim_is_running(self):
        return self._xbot_adapter.sim_is_running()
    ...
    def _parse_env_opts(self):
        ...
        if not rt_opts["use_gpu"]: # don't use GPU at all
            rt_opts["device"]="cpu"
        else: # use GPU
            Journal.log(self.__class__.__name__,
            "_parse_env_opts",
            "GPU not supported yet for RtDeploymentEnv!!",
            LogType.EXCEP,
            throw_when_excep = True)        
    ...
    def _init_world(self):
        ...
        big_info = "Configuring RtDeploymentEnvironment " + self._name + "\n" + \
            "use_gpu: " + str(self._env_opts["use_gpu"]) + "\n" + \
            "device: " + str(self._env_opts["device"])
        Journal.log(self.__class__.__name__,
            "_init_world",
            big_info,
            LogType.STAT,
    ...
    def _configure_scene(self):
        ...
        for i in range(len(self._robot_names)):
            ...
            self._xbot_adapter=RosXbotAdapter(model_name=self._robot_names[0],
            ...
            for jnt in range(len(jnt_names)):
                to_monitor.append((self._robot_names[i],jnt_names[jnt]))
    ...
    def _apply_cmds_to_jnt_imp_control(self, robot_name:str):
        ...
        self._xbot_adapter.setJointsImpedanceCommand(self._jnt_imp_controllers[self._robot_names[0]].get_pvesd())

    def _step_sim(self): 
        time_elapsed=self._xbot_adapter.step()
    ...
    def _set_startup_jnt_imp_gains(self,
        ...
        self._xbot_adapter.apply_joint_impedances(self._jnt_imp_controllers[self._robot_names[0]].get_pvesd())
        self._xbot_adapter.run(duration_sec=1.0)
    ...
    def _get_root_state(self, 
        ...
        self._root_p[robot_name][:, :] = torch.from_numpy(self._xbot_adapter.).reshape(self._num_envs, -1).to(self._dtype)
        self._root_q[robot_name][:, :] = torch.from_numpy(self._xbot_adapter.).reshape(self._num_envs, -1).to(self._dtype)
        ...
        if dt is None:
            ...
            self._root_v[robot_name][:, :] = torch.from_numpy(self._xbot_adapter.).reshape(self._num_envs, -1).to(self._dtype)             
            self._root_omega[robot_name][:, :] = torch.from_numpy(self._xbot_adapter).reshape(self._num_envs, -1).to(self._dtype)        
        else:
    ...
    def _get_robots_jnt_state(self, 
        ...
        self._jnts_q[robot_name][:, :] = torch.from_numpy(self._xbot_adapter.).reshape(self._num_envs, -1).to(self._dtype)

        if dt is None:
            self._jnts_v[robot_name][:, :] = torch.from_numpy(self._xbot_adapter.).reshape(self._num_envs, -1).to(self._dtype)     
        else: 
        ...
        self._jnts_eff[robot_name][env_indxs, :] = torch.from_numpy(self._xbot_adapter.).reshape(self._num_envs, -1).to(self._dtype) 
    ...
    def _init_robots_state(self):
        ...
        for i in range(0, len(self._robot_names)):
            ...
            self._root_p[robot_name] = torch.from_numpy(self._xbot_adapter..copy()).reshape(self._num_envs, -1).to(self._dtype)
            ...
            self._root_q[robot_name] = torch.from_numpy(self._xbot_adapter..copy()).reshape(self._num_envs, -1).to(self._dtype)
            ...
            self._jnts_q[robot_name] = torch.from_numpy(self._xbot_adapter..copy()).reshape(self._num_envs, -1).to(self._dtype)
            ...
            self._root_v[robot_name] = torch.from_numpy(self._xbot_adapter..copy()[0:3]).reshape(self._num_envs, -1).to(self._dtype)
            ...
            self._root_omega[robot_name] = torch.from_numpy(self._xbot_adapter.copy()[3:6]).reshape(self._num_envs, -1).to(self._dtype)
            ...
            self._jnts_v[robot_name] = torch.from_numpy(self._xbot_adapter..copy()).reshape(self._num_envs, -1).to(self._dtype)
            self._jnts_v_default[robot_name] = self._jnts_v[robot_name].clone()
            
            # joints efforts (measured, default)
            self._jnts_eff[robot_name] = torch.from_numpy(self._xbot_adapter..copy()).reshape(self._num_envs, -1).to(self._dtype)
